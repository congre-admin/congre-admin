...
La aplicación frontend consta de **dos apps separadas** que comparten el mismo build de Vite pero tienen ssIds, fuentes de datos y requisitos de acceso distintos:
...
flowchart LR
    ...
    subgraph "App Admin (/admin)"
        AA[AdminApp]
        DS[DataService]
        API[api.gs<br/>doPost]
        CG[GSheet Core<br/>CORE_SS_ID]
        OG[GSheets Operativos]
        AA --> DS --> API
        API --> CG
        API --> OG
    end
...
| Aspecto | Pública (`/`) | Admin (`/admin`) |
|---------|---------------|------------------|
| **ssId utilizado** | `PUBLIC_SS_ID` | `CORE_SS_ID` |
| **Persistido como** | `localStorage: congre_public_ss_id` | `localStorage: congre_admin_ss_id` |
| **Fuente de datos** | GSheet Público (lectura directa) | GAS API (`doPost`) |
| **Protocolo** | `/gviz/tq` (Google Visualization) | JSON POST a GAS |
| **Autenticación** | No requiere | Login + sessionToken |
| **Alcance de datos** | Solo datos "publicados" (subset) | Core completo + módulos operativos |
...
Durante `actionInstall`, se crean **dos spreadsheets** en paralelo:

| Spreadsheet | Formato del nombre | Contenido |
|-------------|-------------------|-----------|
| **Core** | `CongreAdmin-{name}-Core` | Usuarios, Perfiles, Configuracion, Registro_Plugins, Sistema_Migraciones |
| **Público** | `CongreAdmin-{name}-Public` | Indice, Anuncios, Reuniones |

El `PUBLIC_SS_ID` se guarda tanto en las propiedades del script GAS como en la tabla `Configuracion` del Core (clave: `ss_publico`).

## 2. El Núcleo (Core System)
...
## 4. Segmentación Física de Datos
...
## 5. Gestión de Ciclo de Vida (Instalación)
...
## 6. Orquestación y Carga Dinámica (Host & Plugins)
...
## 7. Capa de Persistencia Agnostica (Storage Adapters)
...
## 9. Librería de Componentes (CongreAdmin-UI)
...
## 10. Sincronización y Resolución de Conflictos
...
## 11. Integridad Referencial y Borrado Lógico
...
## 12. Formato de Seed Data (Datos Iniciales)
